...
class TenantModuleAccessMiddleware
{
    ...
    public function handle($request, Closure $next, $module)
    {
        ...
        try {
            ...
            if (
                $this->action === 'close'
                && (! $this->user->hasRole('super admin') && ! $this->_hasCRUDPermissions())
            ) {
                throw new UnauthorizedException();
            }
            ...
            Log::error('Tenant Module Access Middleware Error', [
                'message' => $message,
                'code' => $code,
    ...
    protected function _matchModuleAction($action)
    {
        ...
        if (in_array($action, ['store', 'sendApproval'])) {
            return 'create';
        }
